...
######### to do overall analytics exclude the election and for drawdon use the 15 min pnl
...
for root, dirs, files in os.walk(alldte_pnl_files):
    for file in files:
        ...
        if 'Date' in pnl_df.columns:
            pnl_df['Date'] = pd.to_datetime(pnl_df['Date'], errors='coerce').dt.date
            pnl_df = pnl_df[~((pnl_df['Date'] >= start_exclude) & (pnl_df['Date'] <= end_exclude))]

        elif 'entry_date' in pnl_df.columns:
            pnl_df['entry_date'] = pd.to_datetime(pnl_df['entry_date'], errors='coerce')
            pnl_df = pnl_df[~((pnl_df['entry_date'].dt.date >= start_exclude) &
                                    (pnl_df['entry_date'].dt.date <= end_exclude))]
