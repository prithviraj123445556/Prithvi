...
def createtradesheet(data, data_fut ,TIME):
    ...
    df['Max_Ex_Date'] = deepcopy(df['En_Date'].map(lambda t: t.replace(hour=9, minute=30 , second=0)))
...
def process_data_for_params(mapped_days ,option_data, df, filter_df, start_date, end_date, counter, output_folder_path, resampled, TIME_FRAME, DELTA,TIME):
    """
    ...
    target_list =  [TIME_FRAME, DELTA,TIME]
    ...
    trades = track_trades_opt(df_ts1, result_dict_opt, option_data,TIME_FRAME)
    ...
    strategy_name = f'{stock}_candle_{TIME_FRAME}_delta_{DELTA}_time_{TIME}'
...
def parameter_process(parameter, mapped_days, option_data, df, filter_df, start_date, end_date, counter, output_folder_path):
    TIME_FRAME, DELTA, TIME = parameter
    resampled_df = resample_data(df,TIME_FRAME)
    resampled = resampled_df.dropna() 
    return process_data_for_params(mapped_days ,option_data, df, filter_df, start_date, end_date, counter, output_folder_path, resampled, TIME_FRAME, DELTA, TIME)




#################################### InPEs ######################################################
superset = 'delta_neutral_positional'
stock = 'NIFTY'
option_type = 'ND'
...
if stock == 'NIFTY':
    ...
    option_data_path = rf"/home/newberry4/jay_data/Data/SENSEX/ohlc_with_all_strikes/"
...
log_file = os.path.join(root_path, "trade_logs.log")
...
if __name__ == "__main__":
    ...
    for start_date, end_date in date_ranges: 
        ...
        for file in option_data_files:
            file1 = compare_month_and_year(start_date, extended_end_date_str, file, stock)
            ...
            for TIME_FRAME in candle_time_frame:
                    for DELTA in delta:
                        ...
                        # TARGET = 100000 if TARGET == 'na' else TARGET  # Replace 'na' with 100000
                            parameters.append([TIME_FRAME, DELTA, TIME])
        ...
        parameters = [value for value in parameters if (stock + '_candle_' + str(value[0])  + '_range_' + '_delta_' + str(value[1]).replace('.', ',') + '_time_' + str(value[3]).replace(':', ',') + '_'  + start_date + '_' + end_date) not in existing_values]
        print('Parameters to run :', len(parameters))
        for value in  parameters:
            string =  (stock + '_candle_' + str(value[0])    + '_delta_' + str(value[2]).replace('.', ',') + '_time_' + str(value[3]).replace('.', ',') + '_'  + start_date + '_' + end_date)
            print(string)
