...
def process_profit_point(mapped_days ,option_data, df, filter_df, start_date, end_date, counter, output_folder_path, resampled, TIME_FRAME, HEDGE, TARGET):
    ...
    for day in mapped_days_temp['Date']: 
        ...
        # if atm_put_exit_premium == 0:
        #     filtered_atm_put_exit_premium = option_data[(option_data['StrikePrice'] == atm_strike) & (option_data['Type'] == 'PE') & (option_data['ExpiryDate'] == expiry_date) & (option_data['Date'] == expiry_date)]['Open']
        #     if not filtered_atm_put_exit_premium.empty:
        #         atm_put_exit_premium = filtered_atm_put_exit_premium.iloc[-1]  # Safe access
        #     else:
        #         atm_put_exit_premium = None 
        #     put_exit_time =  pd.to_datetime(f"{expiry_date} 15:20:00")
        ...
        # if atm_call_exit_premium == 0:
        #     filtered_atm_call_exit_premium = option_data[(option_data['StrikePrice'] == atm_strike) & (option_data['Type'] == 'CE') & (option_data['ExpiryDate'] == expiry_date) & (option_data['Date'] == expiry_date)]['Open']
        #     if not filtered_atm_call_exit_premium.empty:
        #         atm_call_exit_premium = filtered_atm_call_exit_premium.iloc[-1]  # Safe access
        #     else:
        #         atm_call_exit_premium = None 
        #     call_exit_time =  pd.to_datetime(f"{expiry_date} 15:20:00")
        ...
        if pd.Timestamp(day).dayofweek != 4:    ## not friday
            ...
            while first_time.time() == pd.to_datetime(check_time).time():
                ...
                if delta in [50,100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600]:
                    quantity = -10 * (delta // 50)            
                    # if new_price > current_price + delta: 
                    if new_price > current_price: 
                        ...
                        # if put_exit_premium == 0:
                        #     filtered_put_exit_premium = option_data[(option_data['StrikePrice'] == put_strike) & (option_data['Type'] == 'PE') & (option_data['ExpiryDate'] == expiry_date) & (option_data['Date'] == expiry_date)]['Open']
                        #     if not filtered_put_exit_premium.empty:
                        #         put_exit_premium = filtered_put_exit_premium.iloc[-1]  # Safe access
                        #     else:
                        #         put_exit_premium = None 
                        #     put_exit_time =  pd.to_datetime(f"{expiry_date} 15:20:00")
                    ...
                    # elif new_price < current_price - delta: 
                        ...
                        # if call_exit_premium == 0:
                        #     filtered_call_exit_premium = option_data[(option_data['StrikePrice'] == call_strike) & (option_data['Type'] == 'CE') & (option_data['ExpiryDate'] == expiry_date) & (option_data['Date'] == expiry_date)]['Open']
                        #     if not filtered_call_exit_premium.empty:
                        #         call_exit_premium = filtered_call_exit_premium.iloc[-1]  # Safe access
                        #     else:
                        #         call_exit_premium = None 
                        #     call_exit_time =  pd.to_datetime(f"{expiry_date} 15:20:00")
        ...
        # while not is_expiry(day, expiry_date) and current_time.time() < pd.to_datetime('15:15:00').time():\\
        while current_time.time() < pd.to_datetime('15:15:00').time():
            ...
            while True:
                match = daily_data[daily_data['Time'] == next_time.time()]
                if not match.empty:
                    new_price = match['Open'].iloc[0]
                    break
                next_time += pd.Timedelta(minutes=1)
                if next_time.time() > pd.to_datetime("15:30").time():
                    print(f"[WARN] No price found after {current_time}. Last tried: {next_time}")
                    new_price = None
                    break
...
output_folder_path = rf'{root_path}/Trade_Sheets/non_split/'
